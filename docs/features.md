...
AFC calls a generic macro, `_AFC_DISPLAY_STATUS`, around toolhead loading and unloading, letting any display integration react without AFC needing to know which display you're using. Define a `gcode_macro _AFC_DISPLAY_STATUS` in your config to forward it to your display of choice; if the macro isn't defined, nothing happens. A working example for the BTT KNOMI, with custom icons for several tool-change states, is available at [rescosta/KNOMI](https://github.com/rescosta/KNOMI/tree/afc-tool-change-icons).
